...
public class DefeatListener {
    ...
    public void onDefeat (BeatTrainerEvent event) throws ObjectMappingException {
        ...
        if (AccountHandler.accountMap.containsKey(uuid)) {
            ...
            for (DefeatMission missions : MissionRegistry.defeatMissions) {
            ...
            if (mission != null) {
                ...
                List<String> locations = mission.getLocations();
                ...
                if (!requirementsEvent.isCanceled()) {
                    ...
                    ComplexConfigManager configManager = BetterMissions.missionConfigManager.get("Defeat");
                    int index = Utils.getIndexFromMissionID("Defeat", mission.getID());
            ...
            if (mission == null) {
                ...
                for (DefeatMission missions : MissionRegistry.permanentDefeatMissions) {
                    ...
                    permanentMissions.removeIf(m -> {
                        ...
                        if (m.equalsIgnoreCase(missions.getID())) {
                            ...
                            List<String> locations = missions.getLocations();
                            String worldName = player.getServerWorld().getWorld().toString().replace("ServerLevel[", "").replace("]", "");
                            BlockPos pos = event.trainer.getPosition();
                            String location = worldName + "," + pos.getX() + "," + pos.getY() + "," + pos.getZ();
                            if (!locations.isEmpty()) {
                            ...
                            if (!requirementsEvent.isCanceled()) {
                                ...
                                ComplexConfigManager configManager = BetterMissions.missionConfigManager.get("Defeat");
                                int index = Utils.getIndexFromMissionID("Defeat", missions.getID());
